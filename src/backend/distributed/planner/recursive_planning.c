...
static void RecursivelyPlanNonColocatedSubqueriesInWhere(Query *query,
														 ...
														 RecursivePlanningContext *
														 recursivePlanningContext);
static void RecursivelyPlanRecurringTupleOuterJoins(Query *query, Node *node,
													RecursivePlanningContext *context);
...
RecursivelyPlanSubqueriesAndCTEs(Query *query, RecursivePlanningContext *context)
{
	 ...
	 * types.
	 */
	RecursivelyPlanRecurringTupleOuterJoins(query, (Node *) query->jointree, context);
 ...
 */
static void
RecursivelyPlanRecurringTupleOuterJoins(Query *query, Node *node,
										RecursivePlanningContext *recursivePlanningContext)
{
	if (node == NULL)
	{
	...
	else if (IsA(node, FromExpr))
	{
		...
		foreach(fromExprCell, fromExpr->fromlist)
		{
			...
			RecursivelyPlanRecurringTupleOuterJoins(query, fromElement,
													recursivePlanningContext);
		}
	...
	else if (IsA(node, JoinExpr))
	{
		...
		RecursivelyPlanRecurringTupleOuterJoins(query, joinExpr->larg,
												recursivePlanningContext);
		RecursivelyPlanRecurringTupleOuterJoins(query, joinExpr->rarg,
												recursivePlanningContext);
